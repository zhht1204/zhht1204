...
var getParameterValue = function (parameterName) {
	...
	if (parameters.length > 1) {
		...
		for (var index in parameters) {
			if (parameterName == parameters[index].split(/\=/)[0]) {
...
var checkCategory = function (categoryPath) {
	var result = [];
	$.ajax({
		url: '/' + categoryPath,
		success: function (callback) {
			var content = $.parseHTML(callback);
			content = $(content[7]).find('tbody').find('td[class="name"]');
			var childCategoryName;
			for (var i = 0; i < content.length; i++) {
				childCategoryName = $(content[i]).find('a').html();
				if (
					'Parent Directory' != childCategoryName &&
					!/\d{4}\//.test(childCategoryName) &&
					!/[1-31]/.test(childCategoryName)
				) {
					result.push(childCategoryName.substring(0, childCategoryName.length - 1));
				}
			}
		},
		async: false
